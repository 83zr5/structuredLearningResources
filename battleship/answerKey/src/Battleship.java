...
public class Battleship {
    ...
    public boolean isSunk(Ship ship) {
        ...
        for (int idx : getShipIndices(pos)) {
            if (!hit[idx]) {
                return false;
            }
        }
    ...
    protected boolean placeShip(ShipPlacement pos) {
        ...
        if (pos.isVertical) {
            if (pos.y + pos.ship.length > BOARD_SIZE) return false;
        } else {
            if (pos.x + pos.ship.length > BOARD_SIZE) return false;
        }
        ...
        for (int idx : indices) {
            if (board[idx] != null) {
                // If there is already a ship on one of
                // these tiles, then this ship can't
                // overlap it
                return false;
            }
        }

        // TODO: write a loop to put the ship into each of it's
        //   tiles on the board.
        for (int idx : indices) {
            board[idx] = pos.ship;
        }
    ...
    protected int[] getShipIndices(ShipPlacement pos) {
        ...
        if (pos.isVertical) {
            for (int i = 0; i < pos.ship.length; i++) {
                indices[i] = (pos.y + i) * BOARD_SIZE + pos.x;
            }
        } else {
            for (int i = 0; i < pos.ship.length; i++) {
                indices[i] = pos.y * BOARD_SIZE + pos.x + i;
            }
        }
        return indices;
    }
